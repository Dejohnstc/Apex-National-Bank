...
export async function failWireTransfer(
  reference: string,
  reason: string
) {
  await dbConnect();
  ...
  try {
    ...
    if (wire.status !== "PROCESSING") {
      throw new Error(
        "Only processing wire transfers can be marked as failed."
      );
    }
...
await updateTransactionStatus({
  ...
  session,
    ...
    await createTransaction({
      ...
      reference: `${wire.reference}-FAILED`,
    ...
    await createNotification({
      ...
      message: `Your wire transfer could not be completed. ${reason}`,
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to mark wire transfer as failed.",
    };
  } finally {
